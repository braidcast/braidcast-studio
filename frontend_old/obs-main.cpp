...
static void create_log_file(fstream &logFile)
{
	...
	get_last_log(false, "obs-multistream/logs", lastLogFile);

	currentLogFile = GenerateTimeDateFilename("txt");
	dst << "obs-multistream/logs/" << currentLogFile.c_str();
	...
	if (logFile.is_open()) {
		delete_oldest_file(false, "obs-multistream/logs");
...
#define LITERAL_SIZE(x) x, (sizeof(x) - 1)
	ostringstream dst;
	dst.write(LITERAL_SIZE("obs-multistream/profiler_data/"));
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	try {
		...
		program.AppInit();
		delete_oldest_file(false, "obs-multistream/profiler_data");
...
static void main_crash_handler(const char *format, va_list args, void * /* param */)
{
	...
	string crashFilePath = "obs-multistream/crashes";
	...
	int ret = MessageBoxA(NULL, finalMessage.c_str(), "OBS MultiStreamer has crashed!",
...
static constexpr char vcRunErrorMsg[] = "OBS MultiStreamer requires a newer version of the Microsoft Visual C++ "
...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		if (arg_is(argv[i], "--multi", "-m")) {
			...
			std::cout << "OBS MultiStreamer - " << App()->GetVersionString(false) << "\n";
			exit(0);
		}
	}
