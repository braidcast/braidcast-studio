...
| §1.5 per-item show/hide transitions | Major | ✅ Fixed `d74d64f3d` (type+dur; behavior owed) |
...
| 1.5 | **Per-item Show/Hide Transitions missing** | `CreateVisibilityTransitionMenu(visible)` — per-item show & hide transition type + duration + properties, copy/paste transition settings (`OBSBasic_Transitions.cpp`) | ✅ **Fixed `d74d64f3d`** (type+duration) — `sceneItems.setShowTransition`/`setHideTransition` (correct transition-source ref lifetime: create-ref released after `set_transition` takes its own; libobs already persists per-item transitions via `scene_save/load_item`); `transitionMenu.ts` Show/Hide submenus (None + type list from `Transitions::TypeList`, preset durations) in both docks. **Deferred sub-gaps:** per-transition properties (Stinger/Fade-color/Luma) + copy/paste transition settings. Behavior owed | Major | `bridge.cpp`, `transitionMenu.ts`, `SourcesDock.svelte`, `CanvasDock.svelte` |
...
| 11.2 | **Arrow-key nudge missing** | Arrows = 1 px, Shift+Arrows = 10 px on the preview (`OBSBasic::Nudge`) | ✅ **Fixed `71ed2b357`** — arrow block in `App.svelte onKeydown` (above the Ctrl gate); `nudge(dx,dy)` reuses `sceneItems.get/setTransform` (partial `{pos}` write), `ARROW_DELTAS` data map, gated on selection + `!isEditable` + `!previewSuspended()` (modal guard). 1px canvas-px, Shift=10px, y-down confirmed | Major | `App.svelte` |
