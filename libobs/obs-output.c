...
static void log_frame_info(struct obs_output *output)
{
	...
	/* Names the mechanism, not a cause. A full send queue has two causes with opposite
	 * fixes -- the local uplink cannot carry the bitrate, or the remote endpoint is not
	 * draining what it is given -- and nothing here can tell them apart. Asserting
	 * "insufficient bandwidth" first sent two investigations after uplink capacity that
	 * a link with sevenfold headroom did not have. */
	if (total && dropped) {
		blog(LOG_INFO,
		     "Output '%s': Number of dropped frames, send queue full "
		     "(local uplink or remote ingest -- not distinguished here): "
		     "%d (%0.1f%%)",
