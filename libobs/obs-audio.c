...
{
	return dup_src && source != dup_src && source->monitoring_type == OBS_MONITORING_TYPE_MONITOR_AND_OUTPUT &&
	       obs_source_dedup_silences(dup_src);
}
		    ...
		    struct audio_output_data *mixes)
{
	...
	/* Held for the tick like the render order, so the owner cannot be freed while it is read. */
