  ...
  <section class="polls" aria-label="Polls">
    <ul bind:this={listEl}>
      {#each polls as p, i (p.id)}
        ...
        {@const finishing = p.finishing === true}
        <li class="poll" data-poll-row={p.id} style:border-left-color={PLATFORM_COLORS[PLATFORM]}>
          <div class="phead">
            ...
            <span class="pstate" class:live={active && !finishing}
              >{finishing ? "Stream ended · getting final results…" : active ? "Poll live" : "Poll ended"}</span
            >
            <span class="pacts">
              {#if active && !finishing}
