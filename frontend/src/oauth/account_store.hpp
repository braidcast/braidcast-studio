...
public:
	...
	// Update only ONE destination's remembered reusable ingest stream id on an existing
	// account (worker-thread safe; never touches tokens). Other destinations' entries are
	// left untouched, so two profiles going live concurrently can't drop each other's
