...
- **`frontend/src/overlay/`** — an embedded HTTP+SSE server (`overlay_server`) serving browser-source stream overlays (ten widget types: alertbox, chatbox, ticker, goalbar, labels, viewercount, followercount, uptime, wheretowatch, chatleaderboard) from `overlay_store` state. Widgets subscribe to four named SSE channels — `chat`, `viewers`, `channels`, `stream` — plus the default `message` channel the alert boxes consume.
