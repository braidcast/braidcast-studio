...
- **`frontend/src/overlay/`** — an embedded HTTP+SSE server (`overlay_server`) serving browser-source stream overlays (alertbox/chatbox/goalbar/labels/ticker/viewercount/followercount) from `overlay_store` state.
